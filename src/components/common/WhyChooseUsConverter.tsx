...
import { Crop, Zap, Ban, Images, HardHat, ShieldPlus } from 'lucide-react';
...
interface WhyChooseUsConverterProps {
  title?: string;
  benefits?: Benefit[];
  className?: string;
}
...
const defaultBenefits: Benefit[] = [
  ...
  {
    icon: <Images className="h-12 w-12 text-blue-500 mx-auto mb-4" />,
    title: "supported_multiple"
  },
  {
...
const WhyChooseUsConverter = ({
  title = "Why Choose Our Image Converter",
  benefits = defaultBenefits,
  className = ""
}: WhyChooseUsConverterProps) => {

  const whyus = useTranslations('whyusConverter');
  const home = useTranslations('home');

  return (
    <section className={`py-12 ${className}`}>
      <div className="container mx-auto px-4">
        <h2 className="text-3xl lg:text-4xl font-bold text-center text-gray-800 mb-10">{home('whyus_converter')}</h2>
...
export default WhyChooseUsConverter;
